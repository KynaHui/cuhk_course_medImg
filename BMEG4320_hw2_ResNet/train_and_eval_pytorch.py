...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run, rank, word_size, init_method):

    lr = 0.0001 #lr in range(0,1), lr = 0.01, 0.1, 1, 10^4, 10^5, 10^6 
    gamma=0.1
    milestones = [0.5 * num_epochs, 0.75 * num_epochs]
    local_rank = int(os.environ["LOCAL_RANK"])
    dist.init_process_group(backend='nccl', init_method=init_method, rank=rank, word_size=word_size)
    ...
    #transforms.AugMix()
    #data_transform = transforms.AugMix(data_transform) #data augmentation, increase robostness & uncertainty
    ...
    train_sampler = dist.DistributedSampler(train_dataset,num_replicas=args.world_size,rank=rank)
    val_sampler = dist.DistributedSampler(val_dataset,num_replicas=args.world_size,rank=rank)
    test_sampler = dist.DistributedSampler(test_dataset,num_replicas=args.world_size,rank=rank)
    ...
    #     model = resnet50(retrained=False, num_classes=n_classes) if resize else ResNet50(in_channels=n_channels, num_classes=n_classes)
    # else:
    #     raise NotImplementedError

    device = torch.device("cuda", local_rank)
    device = DDP(model, device_ids=[local_rank], output_device=gpu_ids)
...
def CE_loss(outputs, targets):
    ...
    # Compute the negative log probabilities for the target class
    neg_log_probs = -torch.log(softmax_outputs.gather(1, targets.view(-1, 1)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs',
                        ...
                        type=int)
    parser.add_argument('--gpu_ids', #local__rank
                        default='0',  # if gpu: 0  else cpu: -1
                        type=str)
    ...
    main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run, rank, word_size, init_method)
